...
class pypykatz:
	...
	def parse_minidump_file(filename, packages = ['all'], chunksize = 10*1024):
		from pypykatz.lsadecryptor.arm64 import is_arm64_dump
		if is_arm64_dump(filename):
			return pypykatz.parse_minidump_file_arm64(filename, packages, chunksize)
	...
	@staticmethod
	def parse_minidump_file_arm64(filename, packages = ['all'], chunksize = 10*1024):
		"""Parse an ARM64 LSASS minidump using structure-scanning heuristics."""
		from pypykatz.lsadecryptor.arm64 import (
			extract_lsa_keys,
			find_logon_session_list,
			find_wdigest_list,
			make_arm64_decryptor,
			_patch_msv,
			_patch_wdigest,
		)
